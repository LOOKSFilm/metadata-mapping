            ...
            for space in spaces:
                data = {
                    ...
                    "filters": [
                        ...
                        {
                            "field": {
                                "custom_field": "field_52",
                                "fixed_field": "CUSTOM_field_52",
                                "group": "SEARCH_ASSETS"
                            },
                            "match": "EQUAL_TO",
                            "search": filmid
                ...
                asearch = login.flow.searchAdvanced(data)
                for match in asearch:
                    clipinfo = login.flow.getClipData(match['clip_id'])
                    clipname = clipinfo.display_name
                    esassetid = clipinfo.asset_id
                    escaptureid = clipinfo.capture_id
                    cprint(str(i)+': Map Data to: "'+clipname+'" on "'+space+'"')
                    try:
                    ...
                    mapfield("field_231", match['clip_id'], esassetid)
